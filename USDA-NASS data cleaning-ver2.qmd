...
strawberry <- strawberry %>%
  ...
  mutate(`Domain Category` = ifelse(!is.na(Use) | !is.na(Name) | !is.na(Code), NA, `Domain Category`))

# We extracted the chemical use, name, and code from the "Domain Category" column. After that, we replaced the original values in "Domain Category" with NA for the rows where the extraction was successful.
```
